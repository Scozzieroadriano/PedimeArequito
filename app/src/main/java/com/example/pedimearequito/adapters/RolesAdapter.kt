...
import com.example.pedimearequito.models.Rol
import com.example.pedimearequito.utils.SharedPref

class RolesAdapter (val context: Activity, val roles: ArrayList <Rol>):RecyclerView.Adapter<RolesAdapter.RolesViewHolders>() {

    val sharedPref = SharedPref (context)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RolesViewHolders {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.cardview_roles,parent,false)
        return RolesViewHolders(view)
    }
    ...
    override fun onBindViewHolder(holder: RolesViewHolders, position: Int) {
    ...
    private fun goToRol(rol: Rol){
        ...
        else if (rol.name == "COMERCIO"){
            sharedPref.save("rol","COMERCIO")
    ...
    class RolesViewHolders (view: View): RecyclerView.ViewHolder(view) {
